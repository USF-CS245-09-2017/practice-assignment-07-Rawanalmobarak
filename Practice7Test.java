...
public class Practice7Test {
	...
	public boolean notStoredTest() {
		...
		try {
			if (!hashtable.containsKey(key)) {
				...
				System.out
						.println("       ALERT: Cannot run \"not present\" test!");
			}
		} catch (Exception e) {
			success = true;
		}
		...
		try {
			...
			for (int i = 0; i < lookingFor.size(); i++) {
				if (!hashtable.get(lookingFor.get(i)).equals(lookingFor.get(i))) {
					System.out
							.println("       failed to get item from hashtable: "
									+ lookingFor.get(i));
					success = false;
				}
			}
	...
	public void runTest() {
		...
		if (time < 200) {
			...
			System.out.println("[    ] Failed timing test -- took " + time
					+ "ms. (Should be less than 170ms.)");
		}
